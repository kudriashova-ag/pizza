...
$('.order').submit(function (e) {
  ...
  if (address != '' && phone != '') {
    let token = '6433595869:AAFxrnVq6tbA1sw9GU2qT6raYs8yIjaYePo';
    let chat = '597037977';
    let message = `Город: ${city},  Адрес: ${address}, Телефон: ${phone}`;
    message += "%0A %0A"
    message += "Заказ: %0A"
    cart.forEach(product => {
      message += `${product.name}, ${product.amount}шт * ${product.price} %0A`
    })
    let url = `https://api.telegram.org/bot${token}/sendMessage?chat_id=${chat}&text=${message}`
    fetch(url)
    cart = [];
    $('.cart').text('')
    showCount();
